  ...
  <!-- END THEME PANEL -->
  <!-- BEGIN PAGE TITLE-->
  <h1 class="page-title">Document Data Entry </h1>
  <!-- BEGIN PAGE BAR -->
  <div class="page-bar">
                          ...
                          <div class="col-md-7 col-sm-12 responsive-1024">
                              ...
                              <?php 
                                  ...
                                  if($uri_segment)
                                  {
                                      ...
                                      $next_idData = $this->Document_model->get_tag_nxt_id($uri_segment, $customerId);

                                      $previous_idData = $this->Document_model->get_tag_previous_id($uri_segment);

                                      if($next_idData)
                                      {
                                         //$nextId = base_url('imagedocument/image_data_entry/'.$next_idData[0]->Id);

                                        if($customerId != '')
                                          {

                                            $nextId = base_url('imagedocument/image_data_entry/'.$next_idData[0]->Id.'/'.$customerId);
                                          }
                                          else
                                          {
                                      ...
                                      if($previous_idData)
                                      {
                                         $previousId = base_url('imagedocument/image_data_entry/'.$previous_idData[0]->Id);
                                      }
                                      else
                                      {
                                         $previousId = '#';
                              ...
                              <iframe src="http://docs.google.com/viewer?url=<?php echo $imgUrl; ?>&amp;embedded=true" width="500" height="400" frameborder="0"></iframe>
                              <!--<iframe src='http://view.officeapps.live.com/op/view.aspx?src=<?php echo $imgUrl; ?>' width='500px' height='400px' frameborder='0'></iframe>-->
                          </div>
                              ...
                              <form id="single_entry" class="coords form-inline">
                                <div id="ajax"><?php if($this->session->flashdata('message')){echo $this->session->flashdata('message');}?></div>
                                  <table class="table table-bordered">
                                      ...
                                      <?php 
                                      /*
                                              if(!empty($tagData->Doc_Url))
                                              {

                                                 $previewUrl = base_url('document/preview/'.$tagData->Id);
                                      ?>
                                     <tr>
                                          <td>
                                              <label class="control-label">Preview/Download Document File</label>
                                          </td>
                                          <td>
                                             <div class="form-group">
                                              
                                              <a href="<?php echo $previewUrl; ?>" target="_blank">Click Here</a>
                                              </div> 
                                            </td>
                                            ...
                                            <td>
                                             <div class="form-group">
                                              ...
                                              <!-- <input type="text" name="etypesingle" class="form-control btn-sm" value = "<?php echo $tagData->Tag_Type; ?>" /> -->
                                               <input type="radio"  name="etypesingle" value="cash+" title="cash+" <?php if ($tagData->Tag_Type == 'cash+') { echo "checked"; } ?> >
                                               <label for="radio1">C+</label>

                                               <input type="radio"  name="etypesingle" value="cash-" title="cash-" <?php if ($tagData->Tag_Type == 'cash-') { echo "checked"; } ?>>
                                               <label for="radio1">C-</label>

                                                <input type="radio"  name="etypesingle" value="bank+"  title="bank+" <?php if ($tagData->Tag_Type == 'bank+') { echo "checked"; } ?>>
                                               <label for="radio1">B+</label>

                                                <input type="radio"  name="etypesingle" value="bank-" title="bank-" <?php if ($tagData->Tag_Type == 'bank-') { echo "checked"; } ?>>
                                               <label for="radio1">B-</label>

                                               <input type="radio"  name="etypesingle" value="other" title="other" <?php if ($tagData->Tag_Type == 'other') { echo "checked"; } ?>>
                                      ...
                                      <tr>
                                          ...
                                          <td>
                                            ...
                                            <div class="form-group">
                                              ...
                                              value = "<?php $ptagName =  $this->Document_model->get_record_by_id('tbl_doc_primary_tag',array('Id'=>$tagData->Primary_Tag));
 ...
 $(document).ready(function(){
    ...
    /****function for load ptag********/
     function loadPtagm (){
            ...
            $.ajax({
                    url:  "<?php echo base_url('imagedocument/load_allPtag'); ?>",
                    type: "POST",
                    async: false,
                    dataType: 'json'
      ...
      function loadStagm (){
          ...
          $.ajax({
                  url:  "<?php echo base_url('imagedocument/load_allStag'); ?>",
                  type: "POST",
                  async: false,
                  dataType: 'json'
...
<script>
    ...
    $("#single_entry").validate({
      ...
      $.ajax({
                cache: false,
                type: "POST",
                url: "<?php echo base_url('imagedocument/single_entry/').$url_string; ?>",
...
<script>
    ...
    $("#multiple_entry").validate({
      ...
      $.ajax({
                cache: false,
                type: "POST",
                url: "<?php echo base_url('imagedocument/multiple_entry/').$url_string; ?>",
