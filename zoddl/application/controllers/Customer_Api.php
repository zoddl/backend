	...
	if (fpv($this->post('Login_Type')) == 'm' && fpv($this->post('Login_Type')) != 'f' && fpv($this->post('Login_Type')) != 'g')
			...
			if ($customer)
				{
					...
					$authtoken = $this->Customer_Api_Model->generateToken($tableName, $customer->Id, $device_token, $device_type);
		...
		if (empty(fpv($this->post('Email_Id'))) || empty(fpv($this->post('Login_Type'))) || empty(fpv($this->post('Social_Id'))))
			...
			if ($socialcustomer)
				...
				$authtoken = $this->Customer_Api_Model->generateToken($tableName, $socialcustomer->Id, $device_token, $device_type);
				$response['ResponseCode'] = "200";
				$response['ResponseMessage'] = 'Customer Logged in Successfully';
...
public function gallerydetailbymonth_post()
  {
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "User Gallery Deatails By Month";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
  ...
  public function searchuserdata_post() 
	{
		...
		if($customerid)
		{
			...
			else
			{
				...
				// $allprimarydata = $this->Customer_Api_Model->get_gallery_search($customerid, $searchData);
				// $allprimarydocdata = $this->Customer_Api_Model->get_gallery_doc_search($customerid, $searchData);

				// $allSearchData = array_merge($allprimarydata,$allprimarydocdata);

				// $response['ResponseCode'] = "200";
				// $response['ResponseMessage'] = 'Search data successfully.';
				// $response['Images'] = $allSearchData;
				// $this->response($response);
			}
		}
		else
...
public function tagtypeseeall_post()
	...
	public function tagtypedetailbymonth_post()
  	...
  	public function updatefirebasetoken_post()
	 {
	 	   /* Api Hit History */

			$Table_Name = "tbl_api_report";
			$Api_Name = "Update Firebase Device Token";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
			...
			if($customerid){
				...
				if ( empty(fpv($this->post('Device_Token'))) )
				{
					$response_data['ResponseCode'] = "400";
					$response_data['ResponseMessage'] = 'Fields are required.';
					$this->response($response_data);
			  ...
			  else
				{
					...
					$response['ResponseCode'] = "200";
					$response['ResponseMessage'] = 'Update Device Token successfully.';					
					$this->response($response);
				}
		}
		else
