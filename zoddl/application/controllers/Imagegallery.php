...
class Imagegallery extends CI_Controller
{
    ...
    public function image_data_entry()
    {
        ...
        $data['tagData'] = $tagData =$this->Gallery_model->get_record_by_id('tbl_tag',array('Id'=>$tagId));
        ...
        {
          $sec_data = $this->Gallery_model->get_record_by_id('tbl_secondary_tag',array('Id'=>$jk));
        ...
        $this->load->view('admin/template/header2',$data);  
        $this->load->view('admin/template/sidebar');        
        $this->load->view('admin/imagegallery/image_data_entry');
        $this->load->view('admin/template/footer2');    
    }
    ...
    public function not_found()
    {
        ...
        $this->load->view('admin/template/header2',$data);  
        $this->load->view('admin/template/sidebar');        
        $this->load->view('admin/imagegallery/not_found');
        $this->load->view('admin/template/footer2'); 
    ...
    public function single_entry()
    {
        ...
        else
        {
            ...
            $tagData =$this->Gallery_model->get_record_by_id('tbl_tag',array('Id'=>$tagId));
           
            /********Ptag data*************/
            $ptagsingle = $this->input->post('ptagsingle');
            $ptagData   =$this->Gallery_model->get_record_by_id('tbl_primary_tag',array('Prime_Name'=>$ptagsingle,'Customer_Id'=>$tagData->Customer_Id));
            if(!empty($ptagData))
            {
                $ptag = $ptagData->Id;
            }
            ...
            {
                $ptagsingle = $this->input->post('ptagsingle');
                $ptagInsert_id =$this->Gallery_model->insert_one_row('tbl_primary_tag', array('Customer_Id'=>$tagData->Customer_Id,'Prime_Name'=>$ptagsingle));
                $ptag = $ptagInsert_id;
            }
            ...
            /********getting stag Id code************/
                ...
                foreach ($staggg as $staggssbb) {
                   $stagData   =$this->Gallery_model->get_record_by_id('tbl_secondary_tag',array('Secondary_Name'=>$staggssbb));
                   if($stagData)
                   {
                       $id_stag= $stagData->Id;
                   }
                   else
                   {
                      ...
                      $id_stag = $this->Gallery_model->insert_one_row('tbl_secondary_tag', array('Customer_Id'=>$tagData->Customer_Id,'Secondary_Name'=>$staggssbb));
                   }
                      $stagId[] = $id_stag;
            ...
            $tagData =$this->Gallery_model->get_record_by_id('tbl_tag',array('Id'=>$tagId));
             ...
             $updatesuccess = $this->db->update('tbl_tag',$data,array('Id'=> $tagId));
             if($updatesuccess)
             {
                 $next_idDataauto = $this->Gallery_model->get_tag_nxt_id($tagId);
                 if($next_idDataauto)
                    {
                       $nextIdauto = base_url('Imagegallery/image_data_entry/'.$next_idDataauto[0]->Id);
                       echo '<script>location.href="'.$nextIdauto.'";</script>';
                    }
                    else
                    {
                       echo '<script>location.href="'.base_url('Imagegallery/image_data_entry/'.$tagId).'";</script>';
                    }
            ...
            $tagData =$this->Gallery_model->get_record_by_id('tbl_tag',array('Id'=>$tagId));
              /********getting ptag Id code*******/
                ...
                foreach ($ptaggg as $ptaggssbb) {
                   $ptagData   =$this->Gallery_model->get_record_by_id('tbl_primary_tag',array('Prime_Name'=>$ptaggssbb,'Customer_Id'=>$tagData->Customer_Id));
                   if($ptagData)
                   {
                       $id_ptag= $ptagData->Id;
                   }
                   ...
                   {
                      $id_ptag = $this->Gallery_model->insert_one_row('tbl_primary_tag', array('Customer_Id'=>$tagData->Customer_Id,'Prime_Name'=>$ptaggssbb));
                   }
                      $ptagId[] = $id_ptag;
              ...
              /********getting stag Id code************/
                ...
                foreach ($staggg as $staggssbb) {
                    ...
                    foreach ($input2 as $stag_values) {
                         $stagData   =$this->Gallery_model->get_record_by_id('tbl_secondary_tag',array('Secondary_Name'=>$stag_values));
                       if($stagData)
                       {
                           $id_stag[]= $stagData->Id;
                       }
                       else
                       {
                          ...
                          $id_stag[] = $this->Gallery_model->insert_one_row('tbl_secondary_tag', array('Customer_Id'=>$tagData->Customer_Id,'Secondary_Name'=>$stag_values));
                       }
              ...
              /********ends getting ptag Id code******/
               ...
               {
               $this->Gallery_model->insert_one_row('tbl_tag', array(
             ...
             echo '<script>location.href="'.base_url('Imagegallery/image_data_entry/'.$tagId.'/'.$tagData->Customer_Id).'";</script>';
        
    }// ends multiple entry function

 //abhinav function load all Ptag

    public function load_allPtag($customerid = '')
    {
        $searchTerm = @$_GET['term'];
        $where = array('Customer_Id' => $customerid);
        $get_allPtag = $this->Gallery_model->get_all_record_by_condition('tbl_primary_tag',$where); 
        $Ptagg = array();
...
//abhinav function load Stag
    ...
    public function load_allStag($customerid = '')
    {
        $searchTerm = @$_GET['term'];
        $where = array('Customer_Id' => $customerid);
        $get_allStag = $this->Gallery_model->get_all_record_by_condition('tbl_secondary_tag',$where);
        $Stagg = array();
