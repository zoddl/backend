...
public function get_total_data_column3_yearmonth_universal($customerid='', $master1='',$master2='',$tag='')
{
  ...
  else if( ($master1 == 'year' && $master2 == 'month' && $tag == 'all') || ($master1 == 'year' && $master2 == 'month' && $tag == 'day')  || ($master1 == 'year' && $master2 == 'month' && $tag == 'count') )
  {
    if($tag == 'all')
	{
		$sql = '(SELECT YEAR(t2.Tag_Send_Date) as Year, DATE_FORMAT(t2.Tag_Send_Date, "%b") as Month, t2.Tag_Type FROM `tbl_doc_tag` t2 WHERE t2.Customer_Id='.$customerid.' GROUP BY t2.Tag_Type ORDER BY t2.Tag_Type)
		UNION
		(SELECT YEAR(t1.Tag_Send_Date) as Year, DATE_FORMAT(t1.Tag_Send_Date, "%b") as Month, t1.Tag_Type FROM `tbl_tag` t1 WHERE t1.Customer_Id='.$customerid.' GROUP BY  t1.Tag_Type ORDER BY t1.Tag_Type)';
	}
	else if($tag == 'day')
	{
