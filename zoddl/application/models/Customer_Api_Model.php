...
}else
{
	...
	public function add_customer($tableName,$Email_Id,$Password,$Login_Type)
	{
     ...
     public function generateToken($tableName, $customerid, $device_token, $device_type)
	{
		$static_str='AT';
		$currenttimeseconds = date("mdY_His");
		$authtoken = $static_str.$customerid.$currenttimeseconds;		
		$authdata = array('Auth_Token' => md5($authtoken), 'device_token' => $device_token, 'device_type' => $device_type);    
...
public function getAllUserDeviceToken(){
	...
	$this->db->select('id, Customer_Name, Email_Id, Phone_No, device_token, device_type');
