...
class Document_model extends CI_Model {
	...
	public function get_date_by_document_image($customerid, $primaryid, $tagdate, $secondarytagid = '')
		{
			...
			if($image->num_rows() > 0)
			{
				...
				foreach($imagedata as $imagedata)
				{
	                   ...
	                   $getdocumentimageData []= '<a href="'.base_url('imagedocument/image_data_entry/'.$imagedata->Id.'/'.$imagedata->Customer_Id).'" target="_blank"><img src="'.$docimageUrl.'" alt="Document" width="32" height="32"></a>';
				...
				foreach($imagedata as $imagedata)
				{
						...
						else
						{
							...
							else
							{
	                         ...
	                         $getdocumentimageData []= '<a href="'.base_url('imagedocument/image_data_entry/'.$imagedata->Id.'/'.$imagedata->Customer_Id).'" target="_blank"><img src="'.$docimageUrl.'" alt="document Img" width="32" height="32"></a>';
    ...
    */
    ...
    public function get_tag_nxt_id($uri_segment, $customerId = '')
	    {
	    	//$query = $this->db->query("Select * from `tbl_doc_tag` where id = (select min(id) from `tbl_doc_tag` where id > $uri_segment and (Tag_Type = '' or Tag_Send_Date = '' or Amount = 0 or Primary_Tag = 0 ))");
	        //return $query->result();

	    	if($customerId === '')
	    	   {
	    	   	    $querydata = $this->db->query("SELECT * FROM `tbl_doc_tag` WHERE 1 = 1 AND Id > $uri_segment AND (Tag_Type = '' or Tag_Send_Date = '' or Amount IS NULL or Primary_Tag = 0 ) ");

	    	   	    if($querydata->num_rows() > 0 )
	        		{

	        			$query = $this->db->query("SELECT * FROM `tbl_doc_tag` WHERE 1 = 1 AND Id > $uri_segment AND (Tag_Type = '' or Tag_Send_Date = '' or Amount IS NULL or Primary_Tag = 0 ) ");
	        		}
	        		else
	        		{
	        			$query = $this->db->query("SELECT * FROM `tbl_doc_tag` WHERE 1 = 1 AND Id < $uri_segment AND (Tag_Type = '' or Tag_Send_Date = '' or Amount IS NULL or Primary_Tag = 0 ) ");
	        	...
	        	else
	        	{
	        		...
	        		$querydata = $this->db->query("SELECT * FROM `tbl_doc_tag` WHERE 1 = 1 AND Customer_Id = $customerId AND (Id > $uri_segment) LIMIT 1");

	        		if($querydata->num_rows() > 0 )
	        		{

	        			$query = $this->db->query("SELECT * FROM `tbl_doc_tag` WHERE 1 = 1 AND Customer_Id = $customerId AND (Id > $uri_segment) LIMIT 1");
