...
public function get_total_data_column3_yearmonth_universal($customerid='', $master1='',$master2='',$tag='')
{
  ...
  else if( ($master1 == 'year' && $master2 == 'day' && $tag == 'all') || ($master1 == 'year' && $master2 == 'all' && $tag == 'day') )
  {
    ...
    $sql = '(SELECT YEAR(t2.Tag_Send_Date) as Year, DATE_FORMAT(t2.Tag_Send_Date, "%d %M %Y") As Date, t2.Tag_Type FROM `tbl_doc_tag` t2 WHERE t2.Customer_Id='.$customerid.' GROUP BY t2.Tag_Send_Date ORDER BY t2.Tag_Send_Date)
    UNION
    (SELECT YEAR(t1.Tag_Send_Date) as Year, DATE_FORMAT(t1.Tag_Send_Date, "%d %M %Y") As Date, t1.Tag_Type FROM `tbl_tag` t1 WHERE t1.Customer_Id='.$customerid.' GROUP BY t1.Tag_Send_Date ORDER BY t1.Tag_Send_Date)';
  ...
  else if( ($master1 == 'month' && $master2 == 'day' && $tag == 'all') || ($master1 == 'month' && $master2 == 'all' && $tag == 'day') )
  {
    ...
    $sql = '(SELECT DATE_FORMAT(t2.Tag_Send_Date, "%b") as Month, DATE_FORMAT(t2.Tag_Send_Date, "%d %M %Y") As Date, t2.Tag_Type FROM `tbl_doc_tag` t2 WHERE t2.Customer_Id='.$customerid.' GROUP BY t2.Tag_Send_Date ORDER BY t2.Tag_Send_Date)
    UNION
    (SELECT DATE_FORMAT(t1.Tag_Send_Date, "%b") as Month, DATE_FORMAT(t1.Tag_Send_Date, "%d %M %Y") As Date, t1.Tag_Type FROM `tbl_tag` t1 WHERE t1.Customer_Id='.$customerid.' GROUP BY t1.Tag_Send_Date ORDER BY t1.Tag_Send_Date)';
