...
class Customer_Api extends REST_Controller {
	...
	if (fpv($this->post('Login_Type')) == 'm' && fpv($this->post('Login_Type')) != 'f' && fpv($this->post('Login_Type')) != 'g')
			...
			if ($customer)
				{
					...
					$authtoken = $this->Customer_Api_Model->generateToken($tableName, $customer->Id, $device_token);
		...
		if (empty(fpv($this->post('Email_Id'))) || empty(fpv($this->post('Login_Type'))) || empty(fpv($this->post('Social_Id'))))
			...
			if ($socialcustomer)
				...
				$authtoken = $this->Customer_Api_Model->generateToken($tableName, $socialcustomer->Id, $device_token);
				$response['ResponseCode'] = "200";
				$response['ResponseMessage'] = 'Customer Logged in Successfully';
	...
	public function _send_user_summary($useremail = '')
	{

				$email = "antaryami@apptology.in";		

			    $to = $email;
				$tablecustomer = 'tbl_customer';
                $subject = 'User Summary Mail';                

                $data['useremail'] = $useremail;

                $data['registrationdate'] = date("Y-m-d");

                $body = $this->load->view('mailers/user-summary-mail', $data, true);
			...
			$adddetail = array();
				if($addcustomer == "You have successfully registered."){
		    		...
		    		if($this->_send_verificationemai(fpv($this->post('Email_Id'))) == TRUE)
					{
						...
						$this->_send_user_summary($this->post('Email_Id'));
		...
		elseif ($this->Customer_Api_Model->exists_email(fpv($this->input->post('Email_Id'))) == 0) {
		...
		else
			{
				if ($token) {                
	                ...
	                $data['resetlink'] = base_url('customerverification/resetpassword/'.$cDetail->Id.'/'.$token);
	                $body = $this->load->view('mailers/customer-forget-mail', $data, true);
	 ...
	 */
     ...
     public function primarytag_post()
	 {
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "Image Gallery Primary Tag List";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
	 ...
	 */
      ...
      public function secondarytag_post()
	 {
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "Image Gallery Secondary Tag List";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
	...
	public function addmanualtag_post()
	 {
         ...
         /* Api Hit History */
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "Image Gallery Add Manual Tag";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
	...
	public function addtag_post()
	 {
	 	 ...
	 	 /* Api Hit History */
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "Image Gallery Add Image Tag";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
	...
	public function gallerytag_post()
	 ...
	 public function homedata_post()
	 {
		...
		if($customerid){
                ...
                /* Report Json start */
                $allreportdata = $this->Customer_Api_Model->get_report_data($customerid);

                $report['Tag_Type'] = "report";
                $report['Images'] = $allreportdata;
...
public function useritemtagdetails_post()
	 {
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "User Image Gallery Item Tag Details";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
	 ...
	 public function useritemedittagdetails_post()
	 {
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "User Image Gallery Item Tag Details Edit";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
  ...
  public function userprimarytagsearchlist_post()
  {
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "User Image Gallery Primary Tag Search List";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
 ...
 public function userprimarytagsearchdetails_post()
  { 
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "User Image Gallery Primary And Secondary Tag Search Details";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
  ...
  public function usersecondarytagsearchlist_post()
  {
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "User Image Gallery Secondary Tag Search List";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
  ...
  public function gallerydetail_post()
  {
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "User Image Gallery Deatails";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
			...
			if($customerid)
			{
					...
					else
					{
						 ...
						 foreach($getsearchTagviewDetails as $key => $gapt)
						 {
						 	...
						 	$images = $this->Customer_Api_Model->get_user_gelleryprimarytag_detail_image($customerid,$primary_tag,$gapt->Date,$page,$limit);
						...
						else
						{
							...
							$tabled = "tbl_doc_tag";
						     ...
						     foreach($getsearchTagviewDetails as $key => $gapt)
							 {
							 	...
							 	$gdate[] = $gapt->Month = date('n',strtotime($gapt->Date));
							 }
							 foreach($getsearchTagviewDetailsdoc as $key => $dapt)
							 {

							 	$ddate[] = $dapt->Month = date('n',strtotime($dapt->Date));
							 }
        					 ...
        					 foreach ($arrayMonth as $key => $value) {
						            ...
						            if ($value == $curentMonth) {
						                
						                $secondaryArray[] = str_pad($value, 2, "0", STR_PAD_LEFT);
						                
						            }
						            else
						            {
						            	$b[] = str_pad($value, 2, "0", STR_PAD_LEFT);

						            }
						        
					        }
					        
					        $c = array_merge($secondaryArray,$b);
  ...
  public function homedatatagtypedetailbymonth_post()
  {
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "Home Page Tag Type Deatails By Month";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
			...
			if($customerid)
			{
				...
				if(fpv($this->post('Tag_Type')) == '' || $month == '')
					{
						$response_data['ResponseCode'] = "400";
					    $response_data['ResponseMessage'] = 'Fields are required.';
					    $this->response($response_data);
					...
					{	
						if($tagtype == 'gallery' || $tagtype == 'document')
						{
							 ...
							 $getsearchTagviewDetails = $this->Customer_Api_Model->get_user_homedatatagtype_detail_by_month($customerid,$tagtype,$getmonthNo);

							
							 	$getsearchTagviewDetails->Month = date('F',strtotime($getsearchTagviewDetails->Date));


							 	$gallmages = $this->Customer_Api_Model->get_user_homedatatagtype_detail_image($customerid,$tagtype,$getsearchTagviewDetails->Date);
							 	
							 	$getsearchTagviewDetails->Images = $gallmages;
						...
						else
						{
							...
							$tabled = "tbl_doc_tag";
						     ...
						     $getsearchTagviewDetailsdoc = $this->Customer_Api_Model->get_user_homedatatagtype_detail_doc_by_month($customerid,$tagtype,$getmonthNo);
						     
							 $gdate[] = $getsearchTagviewDetails->Month = date('n',strtotime($getsearchTagviewDetails->Date));
							 
							 $ddate[] = $getsearchTagviewDetailsdoc->Month = date('n',strtotime($getsearchTagviewDetailsdoc->Date));
...
/**** Filter API ***/
  ...
  public function filteralphabateprimarytag_post()
  {
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "Filter Image Gallery Primary Tag Alphabate";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
  ...
  public function filteralphabatesecondarytag_post()
  {
			...
			$Api_Name = "Filter Image Gallery Secondary Tag Alphabate";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
  ...
  public function filteramountprimarytag_post()
  {
			...
			$Api_Name = "Filter Image Gallery Primary Tag Amount";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
  ...
  public function filteramountsecondarytag_post()
  {
			...
			$Api_Name = "Filter Image Gallery Secondary Tag Amount";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
  ...
  public function filtercountprimarytag_post()
  {
			...
			$Api_Name = "Filter Image Gallery Primary Tag Count";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
  ...
  public function filtercountsecondarytag_post()
  {
			...
			$Api_Name = "Filter Image Gallery Secondary Tag Count";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
  ...
  public function filterusesprimarytag_post()
  {
			...
			$Api_Name = "Filter Image Gallery Primary Tag Uses";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
  ...
  public function filterusessecondarytag_post()
  {
			...
			$Table_Name = "tbl_api_report";			
			$Api_Name = "Filter Image Gallery Secondary Tag Uses";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 			
			Api_Hit($Table_Name, $Api_Name, $Api_Count, $Api_Date, $Api_Date_Int);
  ...
  public function deletetag_post()
	 {
			...
			$Table_Name = "tbl_api_report";			
			$Api_Name = "Delete Image Gallery Single Image";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 			
			Api_Hit($Table_Name, $Api_Name, $Api_Count, $Api_Date, $Api_Date_Int);
  ...
  public function permission_post()
	...
	public function reportprimarytag_post()
	{
		...
		/* Api Hit History */
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "Report Primary Tag Details";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date);
			Api_Hit($Table_Name, $Api_Name, $Api_Count, $Api_Date, $Api_Date_Int);
	 ...
	 public function reportsecondarytag_post()
	 {
	 	...
	 	/* Api Hit History */
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "Report Secondary Tag Details";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date);
			Api_Hit($Table_Name, $Api_Name, $Api_Count, $Api_Date, $Api_Date_Int);
...
public function gallerydetailbymonth_post()
  {
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "User Image Gallery Deatails By Month";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
			...
			if($customerid)
			{
					...
					else
					{
						 ...
						 $getsearchTagviewDetails = $this->Customer_Api_Model->get_user_gelleryprimarytag_detail_by_month($customerid,$primary_tag,$getmonthNo);
						 
						 if(!empty($getsearchTagviewDetails))
						 {
							 $images = $this->Customer_Api_Model->get_user_gelleryprimarytag_detail_image($customerid,$primary_tag,$getsearchTagviewDetails->Date,$page,$limit);
							 	
							 	if(!empty($images))
							 	{

							 		$getsearchTagviewDetails->Primary_Tag = $this->post('Primary_Tag');
								 	$getsearchTagviewDetails->Prime_Name = $this->db->get_where('tbl_primary_tag', array('Id' => $primary_tag))->row()->Prime_Name;
								 	$getsearchTagviewDetails->Month = date('F',strtotime($getsearchTagviewDetails->Date));
  ...
  public function searchuserdata_post() 
	{
		
		/* Api Hit History */
		
			$Table_Name = "tbl_api_report";
			$Api_Name = "Search User Data";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date);
			Api_Hit($Table_Name, $Api_Name, $Api_Count, $Api_Date, $Api_Date_Int);
		...
		$tablecustomer = "tbl_customer";
		$customerid = $this->Customer_Api_Model->get_customer_id($tablecustomer,$customerauthtoken);
		$searchData = fpv($this->post('Searchdata'));
		...
		if($customerid)
		{
			...
			{
				$getallprimarytag = $this->Customer_Api_Model->gallery_primary_tag_search($customerid, $searchData);
				$allprimarytagimagedata = array();
				foreach($getallprimarytag as $key => $gapt) 
				{
					$getallimage = $this->Customer_Api_Model->all_image_data_search($gapt->Primary_Tag, $searchData);
					$gapt->Images = $getallimage;
					array_push($allprimarytagimagedata,$gapt);
				}
	...
	public function toplistprimetagdata_post()
	{
		$customerauthtoken = fpv($this->post('Authtoken'));
		$tablecustomer = "tbl_customer";
		$customerid = $this->Customer_Api_Model->get_customer_id($tablecustomer,$customerauthtoken);
		...
		if($customerid)
		{
			$getAllTagList = array("cash+","cash-","bank+", "bank-","other");

			$allprimarytagimagedata = array();

			foreach($getAllTagList as $key => $value)
			{
				$getList = new stdClass();
				$getAllDataList = $this->Customer_Api_Model->all_prime_topdata_search($customerid,$value);

				$totalDataList = count($getAllDataList);
				if($totalDataList<5){
					for ($i=$totalDataList; $i < 5; $i++) { 
						$getAllDataList[$i]['Prime_Name']="";
						$getAllDataList[$i]['Primary_Tag']="";
						$getAllDataList[$i]['Tag_Type']="";
						$getAllDataList[$i]['Total_Amount']="";
						$getAllDataList[$i]['Total_Count']="";
					}
				}

				$getList->$value = $getAllDataList;
				array_push($allprimarytagimagedata,$getList);
			}

			$response['ResponseCode'] = "200";
			$response['ResponseMessage'] = 'Tag fetched successfully.';
			$response['Payload'] = $allprimarytagimagedata;
			$this->response($response);
		}
		else
		{
...
public function tagtypeseeall_post()
	{
			...
			$Table_Name = "tbl_api_report";
			$Api_Name = "Tag Type See All Deatails";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
			...
			if($customerid)
			{
				...
				if(fpv($this->post('Tag_Type')) == '')
					...
					else
					{
							if($tagtype == 'gallery' || $tagtype == 'document')
							{
						    	$getsearchTagviewDetails = $this->Customer_Api_Model->get_user_homedataprimaryandtagtype_detail($customerid,$tagtype);

								 foreach($getsearchTagviewDetails as $key => $gapt)
								 {

									$gallmages = $this->Customer_Api_Model->get_user_homedataprimaryandtagtype_detail_image($customerid,$tagtype,$gapt->Primary_Tag);
								 ...
								 $getsearchTagviewDetailscustom = $this->Customer_Api_Model->get_user_homedataprimaryandtagtypecustom_detail($customerid,$tagtype);
								 foreach($getsearchTagviewDetailscustom as $key => $gapt)
								 {

									$gallmages = $this->Customer_Api_Model->get_user_homedataprimaryandtagtypecustom_detail_image($customerid,$tagtype,$gapt->Primary_Tag, $gapt->Source_Type);
	...
	public function tagtypedetailbymonth_post()
  	{

  			/* Api Hit History */

			$Table_Name = "tbl_api_report";
			$Api_Name = "User Tag Type Deatails By Month";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
			...
			if($customerid)
			{
				...
				$primarytag = $this->post('Primary_Tag');

				$sourcetype = $this->post('Source_Type');
				

				if(fpv($this->post('Tag_Type')) == '' || fpv($this->post('Primary_Tag')) == '' || fpv($this->post('Source_Type')) == '')
					{
						$response_data['ResponseCode'] = "400";
					    $response_data['ResponseMessage'] = 'Fields are required.';
					    $this->response($response_data);
					...
					{
						 $getsearchTagviewDetails = $this->Customer_Api_Model->get_user_tagtypedetailbymonth_detail($customerid, $primarytag, $tagtype, $sourcetype);

						 foreach($getsearchTagviewDetails as $key => $gapt)
						 {

						 	$gapt->Month = date('F',strtotime($gapt->Date));
						 	$gapt->Images = $this->Customer_Api_Model->get_user_tagtypedetailbymonth_detail_image($customerid,$primarytag, $tagtype, $gapt->Date, $sourcetype);
  	...
  	public function tagtypedetailbyprimarytag_post()
  	{

  			/* Api Hit History */

			$Table_Name = "tbl_api_report";
			$Api_Name = "User Tag Type Deatails By Primar Tag";
			$Api_Count = 1;
			$Api_Date = date("Y-m-d");		
			$Api_Date_Int = strtotime($Api_Date); 
			...
			if($customerid)
			{
				...
				if(fpv($this->post('Tag_Type')) == '' || fpv($this->post('Primary_Tag')) == '' || fpv($this->post('Source_Type')) == '')
					...
					{
						 $getsearchTagviewDetails = $this->Customer_Api_Model->get_user_tagtypedetailbyprimartag_detail($customerid, $primarytag, $tagtype, $sourcetype);
  					 	 
  					 	 $getsearchTagviewDetails->Images = $this->Customer_Api_Model->get_user_tagtypedetailbyprimarytag_detail_image($customerid,$primarytag, $tagtype, $sourcetype);
