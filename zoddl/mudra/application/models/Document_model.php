    ...
    public function get_tag_nxt_id($uri_segment, $customerId = '')
	    {
	        ...
	        if($customerId === '')
	    	   {
	    	   	    $query = $this->db->query("Select * from `tbl_doc_tag` where id > $uri_segment and (Tag_Type = '' or Tag_Date = '' or Amount = 0 or Primary_Tag = 0 or Secondry_Tag = '') ");
	        	}
	        	else
	        	{
	        		$query = $this->db->query("Select * from `tbl_doc_tag` where id = (select min(id) from `tbl_doc_tag` where id > $uri_segment) and Customer_Id = $customerId");
